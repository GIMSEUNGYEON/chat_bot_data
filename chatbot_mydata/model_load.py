...
class PositionalEncoding(tf.keras.layers.Layer):
    def __init__(self, position, d_model, name="positional_encoding", trainable=False, dtype=tf.float32):
        super(PositionalEncoding, self).__init__(name=name, trainable=trainable, dtype=dtype)
    ...
    def get_config(self):
        ...
        return {**base_config, "position": self.position, "d_model": self.d_model, "name": self.name}
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def __init__(self, d_model, num_heads, name="multi_head_attention", trainable=False, dtype=tf.float32):
        super(MultiHeadAttention, self).__init__(name=name, trainable=trainable, dtype=dtype)
...
class MyModel:
    def __init__(self):
        ...
        urllib.request.urlretrieve("https://raw.githubusercontent.com/GIMSEUNGYEON/chat_bot_data/main/chatbotdata2.csv", filename="chatbotdata2.csv")

        data = pd.read_csv('chatbotdata2.csv', sep='|')
        ...
        self.model = load_model("chatbot_data2.h5",custom_objects={'MultiHeadAttention': MultiHeadAttention,'PositionalEncoding':PositionalEncoding, 'CustomSchedule': CustomSchedule, 'loss_function': loss_function})
...
if __name__ == '__main__':
    mm = MyModel()
    output = mm.predict("안녕")
    output = mm.predict("넌 뭐야?")
    output = mm.predict("챗봇, 너 뭐해?")
    output = mm.predict("좋은 하루")
    output = mm.predict("기분이 별로야.")
    output = mm.predict("지금 예약할 수 있는 객실엔 어떤게 있죠")
