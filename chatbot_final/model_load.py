...
class MyModel:
    def __init__(self):
        ...
        self.model = load_model("chatbot.h5",custom_objects={'MultiHeadAttention': MultiHeadAttention,'PositionalEncoding':PositionalEncoding, 'CustomSchedule': CustomSchedule, 'loss_function': loss_function})
...
if __name__ == '__main__':
    mm = MyModel()
    output = mm.predict("12시 땡!")
    output = mm.predict("감기 기운이 있는것 같아")
    output = mm.predict("가족 여행 가볼까?")
    output = mm.predict("그냥 할까?")
    output = mm.predict("나 많이 기대했는데")
    output = mm.predict("놀러가기 딱 좋은 날씨다")
    output = mm.predict("노래방 가면 어색할까")
